...
from ShareEz.auth import ShareEzAuth
...
class ShareEz:
    def __init__(self, auth: ShareEzAuth = None) -> None:
        """
        The ShareEz class is the main SDK class for the ShareEz API. It acts as a wrapper for the various
        API endpoints, providing a simple and intuitive programmatic interface.

        Args:
            auth (int, optional): An instance of the ShareEz auth class, which is used for authentication and authorization with the API. Defaults to None.
        """
        self.auth = auth if auth else ShareEzAuth()
    ...
    def list_datasets(self):
        """
        List all current datasets within ShareEz instance.
    ...
    def wait_for_job_outcome(self, _id: str, interval: int = 1):
        ...
        Raises:
            ShareEz.exceptions.JobFailedException: If the job outcome failed.
        """
        while True:
    ...
    def download_dataframe(
        ...
        Args:
            ...
            query (ShareEz.items.query.Query, optional): An optional query type to provide when downloading data. Defaults to empty.

        Raises:
            DatasetNotFoundException: ShareEz.exceptions.DatasetNotFoundException: If the
                specificed domain, dataset and version to download does not exist in the ShareEz instance
    ...
    def upload_dataframe(
        ...
        Raises:
            ShareEz.exceptions.DataFrameUploadValidationException: If the DataFrame's schema is incorrect.
            ShareEz.exceptions.DataFrameUploadFailedException: If an unexpected error occurs while uploading the DataFrame.
            ShareEz.exceptions.DatasetNotFoundException: If the specified dataset does not exist.
    ...
    def fetch_dataset_info(self, layer: str, domain: str, dataset: str):
        ...
        Raises:
            ShareEz.exceptions.DatasetInfoFailedException: If an error occurs while fetching the dataset information.
            ShareEz.exceptions.DatasetNotFoundException: If the specified dataset does not exist.
    ...
    def convert_dataframe_for_file_upload(self, df: DataFrame):
        ...
        return {
            "file": (
                f"ShareEz-sdk-{int(datetime.now().timestamp())}.parquet",
    ...
    def generate_schema(
        ...
        Raises:
            ShareEz.exceptions.SchemaGenerationFailedException: If an error occurs while generating the schema.

        Returns:
            ShareEz.items.schema.Schema: A Schema class type from the generated schema for the DataFrame and dataset.
        """
        url = (
    ...
    def create_schema(self, schema: Schema):
        ...
        Args:
            schema ShareEz.items.schema.Schema: The schema model for which you want to create for.

        Raises:
            ShareEz.exceptions.SchemaAlreadyExistsException: If you try to create a schema that already exists in ShareEz.
            ShareEz.exceptions.SchemaCreateFailedException: If an error occurs while trying to update the schema.
        """
    ...
    def update_schema(self, schema: Schema):
        ...
        Args:
            schema (ShareEz.items.schema.Schema): The new schema model that will be used for the update.

        Raises:
            ShareEz.exceptions.SchemaUpdateFailedException: If an error occurs while trying to update the schema.
        """
        schema_dict = schema.dict()
        url = f"{self.auth.url}/schema"
    ...
    def create_client(self, client_name: str, client_permissions: list[str]):
        ...
        Raises:
            ShareEz.exceptions.InvalidPermissionsException: If an error occurs while trying to create the client.
        """
    ...
    def delete_client(self, client_id: str) -> None:
        ...
        Raises:
            ShareEz.exceptions.SubjectNotFoundException: If the client does not exist.
        """
    ...
    def create_protected_domain(self, name: str):
        ...
        Raises:
            ShareEz.exceptions.InvalidDomainNameException: If the domain name is invalid.
            ShareEz.exceptions.DomainConflictException: If the domain already exists.
    ...
    def create_user(self, user_name: str, user_email: str, user_permissions: list[str]):
        ...
        Raises:
            ShareEz.exceptions.InvalidPermissionsException: If an error occurs while trying to create the user.
        """
    ...
    def list_subjects(self):
        """
        List all current subjects within ShareEz instance.
    ...
    def list_layers(self):
        """
        List all current layers within ShareEz instance.
    ...
    def list_protected_domains(self):
        """
        List all current protected domains within ShareEz instance.
    ...
    def delete_dataset(self, layer: str, domain: str, dataset: str):
        ...
        Raises:
            ShareEz.exceptions.DatasetNotFoundException: If the dataset does not exist.
        """
        url = f"{self.auth.url}/datasets/{layer}/{domain}/{dataset}"
        response = requests.delete(
            url,
