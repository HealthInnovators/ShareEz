...
from ShareEz.items.schema import Schema, SchemaMetadata
from ShareEz import ShareEz


def upload_and_create_dataset(
    ShareEz: ShareEz, metadata: SchemaMetadata, df: DataFrame, upgrade_schema_on_fail=False
):
    """
    Uploads a dataframe to a dataset in the API, creating schema first if necessary.

    Args:
        ShareEz (ShareEz): An instance of the ShareEz SDK's main class.
    ...
    Raises:
        ShareEz.exceptions.DataFrameUploadValidationException: If the DataFrame's schema is incorrect and upgrade_schema_on_fail is False.
    """
    try:
        ShareEz.upload_dataframe(
        ...
        if upgrade_schema_on_fail:
            update_schema_to_dataframe(ShareEz, metadata, df)
            ShareEz.upload_dataframe(
                metadata.layer,
                metadata.domain,
                metadata.dataset,
...
def update_schema_to_dataframe(
    ...
    """
    schema_response = ShareEz.generate_schema(
        df, metadata.layer, metadata.domain, metadata.dataset, metadata.sensitivity
    )
