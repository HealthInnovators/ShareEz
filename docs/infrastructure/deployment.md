...
For departments that already have an existing infrastructure, we have extracted the top level infrastructure into a single Terraform [module](https://github.com/no10ds/ShareEz/tree/main/infrastructure/modules/ShareEz).

### Usage

```
module "ShareEz" {
  source  = "git@github.com:no10ds/ShareEz.git//infrastructure/modules/ShareEz"
  ...
  resource-name-prefix = "ShareEz-${your-team-name}"
...
- `vpc_id` - VPC id
- `resource-name-prefix` - The prefix that will be given to all of these ShareEz resources, it needs to be unique so to not conflict with any other instance e.g `ShareEz-<your-team/project/dept>`
...
- `catalog_disabled` - if set to `true` it will disable the ShareEz internal data catalogue
- `tags` - if provided, it will tag the resources with the defined value. Otherwise, it will default to "Resource = '
  data-f1-ShareEz'"
- `custom_user_name_regex` - Regex that when supplied usernames must conform to when creating a new user. Defaults to none, in which case ShareEz will default to it's basic username validity checks.
- `task_cpu` - If provided, will update CPU resource allocated to the ECS task running ShareEz instance. Otherwise will default to 256.
- `task_memory` - If provided, will update memory resource allocated to the ECS task running ShareEz instance. Otherwise will default to 512.
...
There are two config files needed to instantiate the ShareEz service, they are `input-params.tfvars` and `backend.hcl`. Please create these with the templates provided, we will add the content shortly.
...
By default, the files are expected to be inside a folder called `ShareEz-infrastructure-config` and positioned on your local machine relative to ShareEz-infrastructure as shown below:
...
```
export ShareEz_INFRA_CONFIG_ENV=../my/relative/location/to/ShareEz/infrastructure
```

If you want to share your infrastructure values across your team then you can turn `ShareEz-infrastructure-config` into a private repo.
...
- `resource-name-prefix` - prefix value for resource names, can be left as ShareEz
