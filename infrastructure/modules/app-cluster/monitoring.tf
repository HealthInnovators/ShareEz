resource "aws_cloudwatch_log_metric_filter" "ShareEz-service-log-error-count" {
  name           = "ShareEz-service-log-error-count"
  pattern        = "ERROR"
  log_group_name = aws_cloudwatch_log_group.log-group.name
  metric_transformation {
    name          = "ShareEz-log-count-errors"
    namespace     = "ShareEz-metric"
    value         = "1"
    default_value = "0"
  }
}
...
resource "aws_cloudwatch_metric_alarm" "log-error-alarm" {
  ...
  metric_name         = "ShareEz-log-count-errors"
  namespace           = "ShareEz-metric"
  period              = "300"
  statistic           = "Sum"
  threshold           = "1"
