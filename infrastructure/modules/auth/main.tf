resource "aws_cognito_user_pool" "ShareEz_user_pool" {
...
resource "aws_cognito_resource_server" "ShareEz_resource_server" {
  identifier = "https://${var.domain_name}"
  name       = "${var.resource-name-prefix}_service_api"

  user_pool_id = aws_cognito_user_pool.ShareEz_user_pool.id
...
resource "aws_cognito_user_pool_client" "test_client" {
  ...
  explicit_auth_flows = var.ShareEz_client_explicit_auth_flows
  allowed_oauth_scopes = [
    "${aws_cognito_resource_server.ShareEz_resource_server.identifier}/CLIENT_APP",
  ]
...
resource "aws_cognito_user_pool_client" "user_login" {
  ...
  generate_secret              = true
  explicit_auth_flows          = var.ShareEz_user_login_client_explicit_auth_flows
  allowed_oauth_scopes = [
    "phone", "email", "openid"
  ]
...
resource "aws_cognito_user_pool_domain" "ShareEz_cognito_domain" {
