...
resource "aws_s3_bucket" "ShareEz_ui" {
...
resource "aws_s3_bucket_ownership_controls" "ShareEz_ui" {
  bucket = aws_s3_bucket.ShareEz_ui.id
  rule {
    object_ownership = "BucketOwnerPreferred"
  }
}

resource "aws_s3_bucket_acl" "ShareEz_ui_storage_acl" {
  bucket     = aws_s3_bucket.ShareEz_ui.id
  acl        = "private"
  depends_on = [aws_s3_bucket_ownership_controls.ShareEz_ui]
}

resource "aws_s3_bucket_website_configuration" "ShareEz_ui_website" {
  bucket = aws_s3_bucket.ShareEz_ui.bucket
...
locals {
  ui_registry_url = "https://github.com/no10ds/ShareEz/releases/download/${var.ui_version}"
}
...
resource "null_resource" "download_static_ui" {
  ...
  provisioner "local-exec" {
    command = templatefile("${path.module}/scripts/ui.sh.tpl", {
      ...
      BUCKET_ID    = aws_s3_bucket.ShareEz_ui.id
    })
  }
}
...
data "aws_iam_policy_document" "s3" {
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        aws_cloudfront_origin_access_identity.ShareEz_ui.iam_arn
      ]
    }
  }
