resource "aws_s3_bucket" "ShareEz_athena_query_results_bucket" {
...
#resource "aws_s3_bucket_lifecycle_configuration" "ShareEz_athena_query_results_bucket_life_cycle" {
#  bucket = aws_s3_bucket.ShareEz_athena_query_results_bucket.id
#
#  rule {
#    id = "expiry_config"
#    status = Enabled
...
resource "aws_s3_bucket_public_access_block" "ShareEz_athena_query_results_bucket" {
  bucket                  = aws_s3_bucket.ShareEz_athena_query_results_bucket.id
...
resource "aws_athena_workgroup" "ShareEz_athena_workgroup" {
  ...
  configuration {
    ...
    result_configuration {
      output_location = "s3://${aws_s3_bucket.ShareEz_athena_query_results_bucket.bucket}"
        ...
        "Statement": [
            {
              ...
              "Effect": "Deny",
              "Resource": [
                "${aws_s3_bucket.ShareEz_athena_query_results_bucket.arn}/*",
                "${aws_s3_bucket.ShareEz_athena_query_results_bucket.arn}"
              ],
              "Condition": {
                "Bool": {
